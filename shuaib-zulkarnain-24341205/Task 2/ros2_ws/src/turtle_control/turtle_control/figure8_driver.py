...
class Figure8Driver(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(Twist, '/turtle1/cmd_vel', 10)  # Creates a publisher to send velocity commands to /turtle1/cmd_vel
        self.subscriber_ = self.create_subscription(Pose, '/turtle1/pose', self.pose_callback, 10)  # to receive pose updates from /turtle1/pose
    ...
    def pose_callback(self, msg):
        self.current_pose = msg # Update the current pose with the latest message
    ...
    def drive_loop(self):
        ...
        msg.linear.x = self.speed   # forward speed
        ...
        if self.state == 'turn_left':
            ...
            if time.time() - self.state_start_time > (2 * math.pi / (self.speed * self.angular_speed_multiplier)):
                 ...
                 self.get_logger().info('State change: turn_right')     # Logs the state change
                 self.state_start_time = time.time()        # reset the timer for the next state
...
def main(args=None):    # Initialize the ROS2 node
